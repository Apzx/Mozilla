...
http.createServer(function (req, res) {
	...
	switch(req.url.split("?")[0]) {
		...
		case "/ask_for_offer":
			console.log("Got asked for offer on key: "+key)
			if (catalog[key] != null) {
				if(catalog[key].offer != null) {
					console.log("Sending pendingOffer...");
					res.writeHead(200, {'Content-Type': 'text/*'});

					res.end('callback('+JSON.stringify(catalog[key].offer)+')');
	...
	setTimeout(function() {
		...
		for (var i in catalog) {
			for (var j in catalog[i]) {
				console.log("\""+i+"\" has an "+j);
			}
		}
	},10);
